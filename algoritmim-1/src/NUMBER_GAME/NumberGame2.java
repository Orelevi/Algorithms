package NUMBER_GAME;
import java.util.Arrays;
import java.util.Scanner;

public class NumberGame2 {

	int arr[];

	///constructor
	public NumberGame2(int[] arr) {
		
		this.arr = arr;
	}

	/// witch asrategi- returns 0 for even and 1 for odd
	public int astrategi (int index1, int index2)
	{
		int sumEven=0;
		int sumOdd=0;

		for(int i=index1; i<index2 ;i+=2)// scanning the array between indexes
		{
			sumEven+=arr[i];
			sumOdd+=arr[i+1];
			System.out.println();
		}

		if (sumEven>sumOdd) return 0;// returning the max
		else return 1;

	}
	
	
/////// if even	
	
	public void gameIfEven (int myPoints, int yourPoints, int i, int j)
	{
		Scanner sc = new Scanner(System.in);
		int myans, answer;
	
	
			if (i%2==0)
				myans=i;
			else myans=j;

			System.out.println("i chous :"+this.arr[myans]) ;
			if (myans==i)i++;
			else j--;
			myPoints+=arr[myans];
			System.out.println("your tern to play, cohus between :" +arr[i]+" or " +arr[j]);
			System.out.println();
			printarrIJ(i, j);
			answer= sc.nextInt();
			if (answer==arr[i])
			{
				yourPoints+=arr[i];
				i++;
				System.out.println("my points: "+myPoints+"   your points: "+yourPoints);
				System.out.println();
				printarrIJ(i, j);
			}
			else
			{
				yourPoints+=arr[j];
				j--;
				System.out.println("my points: "+myPoints+"   your points: "+yourPoints);
				System.out.println();
				printarrIJ(i, j);
			} 

			if (i<j)
			{
				int whatNext= astrategi(i,j);
				if (whatNext==0)
					this.gameIfEven (myPoints, yourPoints,  i,  j);
				else
					this.gameIfOdd (myPoints, yourPoints,  i,  j);
			}
			else this.finalPrint(myPoints, yourPoints);


		}

	///////if odd

	public void gameIfOdd (int myPoints, int yourPoints, int i, int j)
	{
		Scanner sc = new Scanner(System.in);
		int myans, answer;
	
	
			if (i%2==1)
				myans=i;
			else myans=j;

			System.out.println("i chous :"+arr[myans]) ;
			if (myans==i)i++;
			else j--;
			myPoints+=arr[myans];
			System.out.println("your tern to play, cohus between :" +arr[i]+" or " +arr[j]);
			System.out.println();
			printarrIJ(i, j);
			answer= sc.nextInt();
			if (answer==arr[i])
			{
				yourPoints+=arr[i];
				i++;
				System.out.println("my points: "+myPoints+"   your points: "+yourPoints);
				System.out.println();
				printarrIJ(i, j);

			}
			else
			{
				yourPoints+=arr[j];
				j--;
				System.out.println("my points: "+myPoints+"   your points: "+yourPoints);
				System.out.println();
				printarrIJ(i, j);
	
			} 

			if (i<j)
			{
				int whatNext= astrategi(i,j);
				if (whatNext==0)
					this.gameIfEven (myPoints, yourPoints,  i,  j);
				else
					this.gameIfOdd (myPoints, yourPoints,  i,  j);
			}
			else this.finalPrint(myPoints, yourPoints);
	
		}

	
	// prints the  array between i-j
	public void printarrIJ (int index1, int index2)
	{
		for( int i=index1; i<=index2; i++)
			System.out.print(arr[i]+", ");
	}

	
	
	/// the game!!!!
	public void CleverGame ()
	{
		int i=0; int j=arr.length-1;
		int myans, answer;
		int myPoints=0; 
		int yourPoints=0;
		
		System.out.println(Arrays.toString(arr));
		System.out.println("i play first");
		
		if (i<j)
		{
			int whatNext= astrategi(i,j);
			if (whatNext==0)
				this.gameIfEven (myPoints, yourPoints,  i,  j);
			else
				this.gameIfOdd (myPoints, yourPoints,  i,  j);
		}
		else this.finalPrint(myPoints, yourPoints);
	
	}
	
	public void finalPrint(int myPoints, int yourPoints)
	{
		   if (myPoints>yourPoints)
		   System.out.println("end of game! i win :)");
		   else if (myPoints==yourPoints)
			   System.out.println("end of game! TEKO");
		   else if (yourPoints>myPoints)
			   System.out.println("you win, bug in program");
		   else
			   System.out.println("end of game! you win :(");
			   
	}


	public static void main(String[] args) {

		int a[]={1,2,30,10};
		NumberGame2 game1= new NumberGame2(a);
		game1.CleverGame();
	}
}






